...
class UserSoundMixerView(disnake.ui.View):
    """Спрощений саундпад для звичайних користувачів (без зміни системної гучності)."""
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        if inter.author.id != self.user_id:
            await inter.response.send_message("❌ This menu is controlled by someone else!", ephemeral=True)
            return False
        return True
    ...
    def build_embed(self) -> disnake.Embed:
        embed = disnake.Embed(
            ...
            description=(
                ...
                f"• **Playing:** `{self.last_sound}`\n"
                f"• **Volume:** `{int(self.volume * 100)}%`\n\n"
                "• **Menu:** Select track from dropdown\n"
                "• 🎲 **Random:** Play a random sound\n"
                "• ⏹️ **Stop:** Stop playback and leave"
            ),
    ...
    async def play_sound(self, inter: disnake.MessageInteraction, file_name: str):
        if not inter.author.voice or not inter.author.voice.channel:
            await inter.response.send_message("⚠️ Connect to a voice channel first!", ephemeral=True)
            return

        file_path = os.path.join(SOUND_DIR, file_name)
        if not os.path.exists(file_path):
            await inter.response.send_message("❌ File not found on the server!", ephemeral=True)
            return
        ...
        if not voice_client.is_connected():
            await inter.response.send_message("❌ Voice connection failed. Try again!", ephemeral=True)
            return
    ...
    async def random_btn(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        files = get_sound_files()
        if not files:
            await inter.response.send_message("❌ No sound files found!", ephemeral=True)
            return
...
class SoundMixerView(disnake.ui.View):
    """Повноцінний мікшер для адміністратора: зациклення, налаштування та збереження гучності."""
    ...
    async def play_sound(self, inter: disnake.MessageInteraction, file_name: str):
        ...
        file_path = os.path.join(SOUND_DIR, file_name)
        if not os.path.exists(file_path):
            await inter.response.send_message("❌ File not found!", ephemeral=True)
            return
    ...
    async def random_track(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        files = get_sound_files()
        if not files:
            await inter.response.send_message("❌ No files available!", ephemeral=True)
            return
        await self.play_sound(inter, random.choice(files))
    ...
    async def repeat_track(self, button: disnake.ui.Button, inter: disnake.MessageInteraction):
        if not self.current_file:
            await inter.response.send_message("⚠️ Play a track first!", ephemeral=True)
            return
